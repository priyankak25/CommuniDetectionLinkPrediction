...
#
# I have implemented community detection and link prediction algorithms using Facebook "like" data.
#
...
def bfs(graph, root, max_depth):
   ...
   """
    ...
    node2check.append(root)
    visited.append(root)

    while len(node2check) != 0:
        node = node2check.popleft()
        visited.append(node)
        if node2distances[node] >= max_depth:
            ...
            for n in graph.neighbors(node):
                if not (n in visited):
                    if not (n in node2check):
                        node2distances[n] = node2distances[node] + 1
                        node2parents[n] = [node]
                        node2num_paths[n] = len(node2parents[n])
                        node2check.append(n)
                    elif n in node2check and node2distances[n] != node2distances[node]:
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    ###TODO
    node2credit = defaultdict(lambda:1)
    edge2credit = {}
    
    for i in reversed(list(node2distances)):
        if i not in node2parents:
            break            
        parents = len(node2parents[i])
        for j in node2parents[i]:
            if i<j:
                edge2credit[(i,j)] = float(node2credit[i]/parents)          
                node2credit[j]+=edge2credit[(i,j)]
            else:
                edge2credit[(j,i)] = float(node2credit[i]/parents)          
                node2credit[j]+=edge2credit[(j,i)]                              
...
def partition_girvan_newman(graph, max_depth):
    ...
    subgraph = [i for i in nx.connected_component_subgraphs(graph)]
    appr_btwness = list(sorted(approximate_betweenness(graphcopy, max_depth).items(), key=lambda x:(-x[1], x[0])))
    
    for i in range(0, len(appr_btwness)):
        if len(subgraph) > 1:
            break
        graphcopy.remove_edge(*appr_btwness[i][0])
        subgraph = [j for j in nx.connected_component_subgraphs(graphcopy)]
...
def cut(S, T, graph):
    ...
    for i in total_edges:
        if ((i[0] in S) and (i[1] in T)):
            cut_cnt+= 1
        elif((i[0] in T) and (i[1] in S)):
            cut_cnt+=1
...
def norm_cut(S, T, graph):
    ...
    ncv = (((cut(S,T,graph))/Svolume) + ((cut(S,T,graph))/Tvolume))    
...
def brute_force_norm_cut(graph, max_size):
    ...
    for i in range(1,max_size+1):
        for j in combinations(graph.edges(),i):
            ...
            if len(components) == 2:
                ncv = norm_cut(components[0],components[1],graph)
                result.append((ncv,list(j)))
            graphcopy.add_edges_from(j)
...
def make_training_graph(graph, test_node, n):
    ...
    neighbor_nodes = sorted(graphcopy.neighbors(test_node))
    for i in range(n):
        graphcopy.remove_edge(*(test_node, neighbor_nodes[i]))
...
def jaccard(graph, node, k):
    ...
    for i in nodes:
        if((i not in neighbor) & (i != node)):
            neighbor1 = set(graph.neighbors(i))
            score = ((node, i), 1. * len(neighbor & neighbor1) / len(neighbor | neighbor1))
            jc.append(score)
            
    return sorted(jc, key=lambda x: (-x[1],x[0][1]))[:k]        
...
def main():
    """
    FYI: This takes ~10-15 seconds to run.
    """
    download_data()
    graph = read_graph()
